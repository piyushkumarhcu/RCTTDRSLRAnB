...
void processOutLinkSLRB(tower_t inTower[6], towerHCAL inHCALTower[12], Cluster inCluster, ap_uint<384> &outLink){
        ...
        for(loop oLink=0; oLink<12; oLink++){
                #pragma HLS UNROLL
                ap_uint<32> end = start + 9;
                outLink.range(end, start) = inHCALTower[oLink].et;
                start += 10;
        }

        ap_uint<32> stop = start + 59;

        outLink.range(stop, start) = inCluster;
}

ecalRegion_t initStructureSLRB(crystal temporary[CRYSTAL_IN_ETA][CRYSTAL_IN_PHI]){
...
ecaltp_t bestOf2SLRB(const ecaltp_t& ecaltp0, const ecaltp_t& ecaltp1) {
        ecaltp_t x;
...
ecaltp_t getPeakBin20NSLRB(const etaStrip_t& etaStrip){
//#pragma HLS PIPELINE II=2
#pragma HLS latency min=4

ecaltp_t best01 = bestOf2SLRB(etaStrip.cr0,etaStrip.cr1) ;
ecaltp_t best23 = bestOf2SLRB(etaStrip.cr2,etaStrip.cr3) ;
ecaltp_t best45 = bestOf2SLRB(etaStrip.cr4,etaStrip.cr5) ;
ecaltp_t best67 = bestOf2SLRB(etaStrip.cr6,etaStrip.cr7) ;
ecaltp_t best89 = bestOf2SLRB(etaStrip.cr8,etaStrip.cr9) ;
ecaltp_t best1011 = bestOf2SLRB(etaStrip.cr10,etaStrip.cr11) ;
ecaltp_t best1213 = bestOf2SLRB(etaStrip.cr12,etaStrip.cr13) ;
ecaltp_t best1415 = bestOf2SLRB(etaStrip.cr14,etaStrip.cr15) ;
ecaltp_t best1617 = bestOf2SLRB(etaStrip.cr16,etaStrip.cr17) ;
ecaltp_t best1819 = bestOf2SLRB(etaStrip.cr18,etaStrip.cr19) ;

ecaltp_t best0123 = bestOf2SLRB(best01,best23) ;
ecaltp_t best4567 = bestOf2SLRB(best45,best67) ;
ecaltp_t best891011 = bestOf2SLRB(best89,best1011) ;
ecaltp_t best12131415 = bestOf2SLRB(best1213,best1415) ;
ecaltp_t best16171819 = bestOf2SLRB(best1617,best1819) ;

ecaltp_t best01234567 = bestOf2SLRB(best0123,best4567) ;
ecaltp_t best89101112131415 = bestOf2SLRB(best891011,best12131415) ;

ecaltp_t best0to15 = bestOf2SLRB(best01234567,best89101112131415) ;
ecaltp_t bestOf20 = bestOf2SLRB(best0to15,best16171819) ;

return bestOf20 ;
}

crystalMax getPeakBin15NSLRB(const etaStripPeak_t& etaStrip){
//#pragma HLS PIPELINE II=2
#pragma HLS latency min=4

crystalMax x;

ecaltp_t best01 = bestOf2SLRB(etaStrip.pk0,etaStrip.pk1) ;
ecaltp_t best23 = bestOf2SLRB(etaStrip.pk2,etaStrip.pk3) ;
ecaltp_t best45 = bestOf2SLRB(etaStrip.pk4,etaStrip.pk5) ;
ecaltp_t best67 = bestOf2SLRB(etaStrip.pk6,etaStrip.pk7) ;
ecaltp_t best89 = bestOf2SLRB(etaStrip.pk8,etaStrip.pk9) ;
ecaltp_t best1011 = bestOf2SLRB(etaStrip.pk10,etaStrip.pk11) ;
ecaltp_t best1213 = bestOf2SLRB(etaStrip.pk12,etaStrip.pk13) ;

ecaltp_t best0123 = bestOf2SLRB(best01,best23) ;
ecaltp_t best4567 = bestOf2SLRB(best45,best67) ;
ecaltp_t best891011 = bestOf2SLRB(best89,best1011) ;
ecaltp_t best121314 = bestOf2SLRB(best1213,etaStrip.pk14) ;

ecaltp_t best01234567 = bestOf2SLRB(best0123,best4567);
ecaltp_t best891011121314 = bestOf2SLRB(best891011,best121314) ;

ecaltp_t bestOf15 = bestOf2SLRB(best01234567,best891011121314) ;
...
void getTowerEtSLRB(crystal tempX[CRYSTAL_IN_ETA][CRYSTAL_IN_PHI], ap_uint<12> towerEt[12]){
        #pragma HLS PIPELINE II=6
        #pragma HLS latency min=4
...
clusterInfo getClusterPosSLRB(const ecalRegion_t& ecalRegion){
#pragma HLS latency min=4
        ...
        clusterInfo cluster ;
                ...
                etaStripPeak.pk0 = getPeakBin20NSLRB(ecalRegion.etaStrip0);
                etaStripPeak.pk1 = getPeakBin20NSLRB(ecalRegion.etaStrip1);
                etaStripPeak.pk2 = getPeakBin20NSLRB(ecalRegion.etaStrip2);
                etaStripPeak.pk3 = getPeakBin20NSLRB(ecalRegion.etaStrip3);
                etaStripPeak.pk4 = getPeakBin20NSLRB(ecalRegion.etaStrip4);
                etaStripPeak.pk5 = getPeakBin20NSLRB(ecalRegion.etaStrip5);
                etaStripPeak.pk6 = getPeakBin20NSLRB(ecalRegion.etaStrip6);
                etaStripPeak.pk7 = getPeakBin20NSLRB(ecalRegion.etaStrip7);
                etaStripPeak.pk8 = getPeakBin20NSLRB(ecalRegion.etaStrip8);
                etaStripPeak.pk9 = getPeakBin20NSLRB(ecalRegion.etaStrip9);
                etaStripPeak.pk10 = getPeakBin20NSLRB(ecalRegion.etaStrip10);
                etaStripPeak.pk11 = getPeakBin20NSLRB(ecalRegion.etaStrip11);
                etaStripPeak.pk12 = getPeakBin20NSLRB(ecalRegion.etaStrip12);
                etaStripPeak.pk13 = getPeakBin20NSLRB(ecalRegion.etaStrip13);
                etaStripPeak.pk14 = getPeakBin20NSLRB(ecalRegion.etaStrip14);

        crystalMax peakIn15;
        peakIn15 = getPeakBin15NSLRB(etaStripPeak);
...
Cluster packClusterSLRB(ap_uint<15>& clusterEt, ap_uint<5>& etaMax_t, ap_uint<5>& phiMax_t, ap_uint<15>& et5x5_t, ap_uint<15>& et2x5_t, ap_uint<2>& brems_t ){
...
void RemoveTmpSLRB(crystal temp[CRYSTAL_IN_ETA][CRYSTAL_IN_PHI], ap_uint<5> seed_eta,  ap_uint<5> seed_phi, ap_uint<2> brems  ){
#pragma HLS ARRAY_PARTITION variable=temp complete dim=0
...
clusterInfo getBremsValPosSLRB(crystal tempX[CRYSTAL_IN_ETA][CRYSTAL_IN_PHI], ap_uint<5> seed_eta,  ap_uint<5> seed_phi ){
#pragma HLS ARRAY_PARTITION variable=tempX complete dim=0
#pragma HLS latency min=6
...
clusterInfo getBremsValNegSLRB(crystal tempX[CRYSTAL_IN_ETA][CRYSTAL_IN_PHI], ap_uint<5> seed_eta,  ap_uint<5> seed_phi ){
#pragma HLS ARRAY_PARTITION variable=tempX complete dim=0
#pragma HLS latency min=6
...
clusterInfo getClusterValSLRB(crystal tempX[CRYSTAL_IN_ETA][CRYSTAL_IN_PHI], ap_uint<5> seed_eta,  ap_uint<5> seed_phi ){
#pragma HLS ARRAY_PARTITION variable=tempX complete dim=0
#pragma HLS latency min=6
...
Cluster getRegion3x4SLRB(crystal temp[CRYSTAL_IN_ETA][CRYSTAL_IN_PHI], ap_uint<5> eta_offset){
#pragma HLS ARRAY_PARTITION variable=temp complete dim=0
//#pragma HLS PIPELINE II=2
...
ecalRegion_t ecalRegion;
        ...
        ecalRegion = initStructureSLRB(temp) ;

        cluster_tmp = getClusterPosSLRB(ecalRegion) ;

        ap_uint<5> seed_phi = cluster_tmp.phiMax ;
        ap_uint<5> seed_eta = cluster_tmp.etaMax ;

        cluster_tmpCenter = getClusterValSLRB(temp, seed_eta, seed_phi) ;
        cluster_tmpBneg = getBremsValNegSLRB(temp, seed_eta, seed_phi) ;
        cluster_tmpBpos = getBremsValPosSLRB(temp, seed_eta, seed_phi) ;
...
//eta, phi, seed energy in cluster_tmp; energy and brems in cluster_tmp1
        ...
        forCluster = packClusterSLRB(cluster_tmp.energy, cluster_tmp.etaMax, cluster_tmp.phiMax, cluster_tmp.et5x5, cluster_tmp.et2x5, cluster_tmp.brems);

        RemoveTmpSLRB(temp, seed_eta, seed_phi, cluster_tmp.brems ) ;
...
void stitchClustersSLRB(Cluster clusterIn[Nbclusters], Cluster clusterOut[Nbclusters]){
...
void algo_top_SLRB(ap_uint<384> link_in[N_INPUT_LINKS], ap_uint<384> link_out[N_OUTPUT_LINKS]){
#pragma HLS ARRAY_PARTITION variable=link_in complete dim=0
#pragma HLS ARRAY_PARTITION variable=link_out complete dim=0
#pragma HLS PIPELINE II=6
#pragma HLS INTERFACE ap_ctrl_hs port=return

#pragma HLS latency min=230
...
processInLinkSLRB(link_in, ECALRegion3x4_1, ECALRegion3x4_2, ECALRegion3x4_3, HCALRegion3x4_1, HCALRegion3x4_2, HCALRegion3x4_3, HCALRegion3x4_4, HCALRegion3x4_5, HCALRegion3x4_6) ;
...
#pragma HLS ARRAY_PARTITION variable=towerEt complete dim=0
        ...
        sort_clusterIn[0] = getRegion3x4SLRB(ECALRegion3x4_1,0);
        sort_clusterIn[1] = getRegion3x4SLRB(ECALRegion3x4_1,0);
        sort_clusterIn[2] = getRegion3x4SLRB(ECALRegion3x4_1,0);
        sort_clusterIn[3] = getRegion3x4SLRB(ECALRegion3x4_1,0);

        sort_clusterIn[4] = getRegion3x4SLRB(ECALRegion3x4_2,3);
        sort_clusterIn[5] = getRegion3x4SLRB(ECALRegion3x4_2,3);
        sort_clusterIn[6] = getRegion3x4SLRB(ECALRegion3x4_2,3);
        sort_clusterIn[7] = getRegion3x4SLRB(ECALRegion3x4_2,3);

        sort_clusterIn[8] = getRegion3x4SLRB(ECALRegion3x4_3,6);
        sort_clusterIn[9] = getRegion3x4SLRB(ECALRegion3x4_3,6);
        sort_clusterIn[10] = getRegion3x4SLRB(ECALRegion3x4_3,6);
        sort_clusterIn[11] = getRegion3x4SLRB(ECALRegion3x4_3,6);

// stitch clusters

        
        stitchClustersSLRB(sort_clusterIn, sort_clusterInStitched);

// sorting of clusters

        bitonicSort16_SLRB(sort_clusterInStitched, sort_clusterOut);
...
// keep only 6 highest clusters, others return back to towers
        ...
        getTowerEtSLRB(ECALRegion3x4_1, towerEtECAL1);
        getTowerEtSLRB(ECALRegion3x4_2, towerEtECAL2);
        getTowerEtSLRB(ECALRegion3x4_3, towerEtECAL3);
...
// from SLRB we have 6 sort_clusterOut and 36 towerEt
       ...
       towerHCAL HCALtower[72];
       #pragma HLS ARRAY_PARTITION variable=HCALtower complete dim=0

       for(loop i=0 ; i<12; i++){
       ap_uint<6> first, second ;
       first = i/4; second = i%4 ;
       HCALtower[i] = HCALRegion3x4_1[first][second]; 
       HCALtower[i+12] = HCALRegion3x4_2[first][second]; 
       HCALtower[i+24] = HCALRegion3x4_3[first][second]; 
       HCALtower[i+36] = HCALRegion3x4_4[first][second]; 
       HCALtower[i+48] = HCALRegion3x4_5[first][second]; 
       HCALtower[i+60] = HCALRegion3x4_6[first][second]; 
       }

        towerHCAL InHCALTowerLink0[12];
        towerHCAL InHCALTowerLink1[12];
        towerHCAL InHCALTowerLink2[12];
        towerHCAL InHCALTowerLink3[12];
        towerHCAL InHCALTowerLink4[12];
        towerHCAL InHCALTowerLink5[12];

        tower_t InTowerLink0[6];
        tower_t InTowerLink1[6];
        tower_t InTowerLink2[6];
        tower_t InTowerLink3[6];
        tower_t InTowerLink4[6];
        tower_t InTowerLink5[6];

        #pragma HLS ARRAY_PARTITION variable=InHCALTowerLink0 complete dim=0
        #pragma HLS ARRAY_PARTITION variable=InHCALTowerLink1 complete dim=0
        #pragma HLS ARRAY_PARTITION variable=InHCALTowerLink2 complete dim=0
        #pragma HLS ARRAY_PARTITION variable=InHCALTowerLink3 complete dim=0
        #pragma HLS ARRAY_PARTITION variable=InHCALTowerLink4 complete dim=0
        #pragma HLS ARRAY_PARTITION variable=InHCALTowerLink5 complete dim=0
        ...
        #pragma HLS ARRAY_PARTITION variable=InTowerLink2 complete dim=0
        #pragma HLS ARRAY_PARTITION variable=InTowerLink3 complete dim=0
        #pragma HLS ARRAY_PARTITION variable=InTowerLink4 complete dim=0
        #pragma HLS ARRAY_PARTITION variable=InTowerLink5 complete dim=0

        for(loop oLink=0; oLink<12; oLink++){
                #pragma HLS unroll
                InHCALTowerLink0[oLink] = HCALtower[oLink];
                InHCALTowerLink1[oLink] = HCALtower[oLink+12];
                InHCALTowerLink2[oLink] = HCALtower[oLink+24];
                InHCALTowerLink3[oLink] = HCALtower[oLink+36];
                InHCALTowerLink4[oLink] = HCALtower[oLink+48];
                InHCALTowerLink5[oLink] = HCALtower[oLink+60];
        }

        for(loop oLink=0; oLink<6; oLink++){
                #pragma HLS unroll
                InTowerLink0[oLink] = towerEt[oLink];
                InTowerLink1[oLink] = towerEt[oLink+6];
                InTowerLink2[oLink] = towerEt[oLink+12];
                InTowerLink3[oLink] = towerEt[oLink+18];
                InTowerLink4[oLink] = towerEt[oLink+24];
                InTowerLink5[oLink] = towerEt[oLink+30];
        }
        ...
        /*---------------------------------link 0------------------------------------*/
       	processOutLinkSLRB(InTowerLink0, InHCALTowerLink0, sort_clusterOut[10], link_out[0]);

        /*---------------------------------link 1------------------------------------*/
        processOutLinkSLRB(InTowerLink1, InHCALTowerLink1, sort_clusterOut[11], link_out[1]);

        /*---------------------------------link 2------------------------------------*/
        processOutLinkSLRB(InTowerLink2, InHCALTowerLink2, sort_clusterOut[12], link_out[2]);

        /*---------------------------------link 3------------------------------------*/
       	processOutLinkSLRB(InTowerLink3, InHCALTowerLink3, sort_clusterOut[13], link_out[3]);

        /*---------------------------------link 4------------------------------------*/
        processOutLinkSLRB(InTowerLink4, InHCALTowerLink4, sort_clusterOut[14], link_out[4]);

        /*---------------------------------link 5------------------------------------*/
        processOutLinkSLRB(InTowerLink5, InHCALTowerLink5, sort_clusterOut[15], link_out[5]);
